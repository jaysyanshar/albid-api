...
router.post('/', (req, res) => {
    ...
    .then(doc => {
        ...
        if (doc.jenisPemeriksaan != rekamMedisEnums.pemeriksaan.soap3) {
            throw Error('Invalid reference ID.')
        }
        else {
            SOAP3.findOne({ idRekamMedis: doc._id }, 'idRekamMedis')
            .then(doc => {
                if (doc == undefined) {
                    crud.createOne(req, res, SOAP3)
                }
...
router.put('/', (req, res) => { crud.updateOne(req, res, SOAP3) })
router.get('/', (req, res) => { crud.readOne(req, res, SOAP3) })
