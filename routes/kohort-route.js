...
router.post('/', (req, res) => {
    ...
    .then(doc => {
        ...
        if (doc.jenisPemeriksaan != rekamMedisEnums.pemeriksaan.kohort) {
            throw Error('Invalid reference ID.')
        }
        else {
            Kohort.findOne({ idRekamMedis: doc._id }, 'idRekamMedis')
            .then(doc => {
                if (doc == undefined) {
                    crud.createOne(req, res, Kohort)
                }
...
router.put('/', (req, res) => { crud.updateOne(req, res, Kohort) })
router.get('/', (req, res) => { crud.readOne(req, res, Kohort) })
