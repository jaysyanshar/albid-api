...
router.post('/', (req, res) => {
    ...
    .then(doc => {
        ...
        if (doc.jenisPemeriksaan != rekamMedisEnums.pemeriksaan.soap1) {
            throw Error('Invalid reference ID.')
        }
        else {
            SOAP1.findOne({ idRekamMedis: doc._id }, 'idRekamMedis')
            .then(doc => {
                if (doc == undefined) {
                    crud.createOne(req, res, SOAP1)
                }
...
router.put('/', (req, res) => { crud.updateOne(req, res, SOAP1) })
router.get('/', (req, res) => { crud.readOne(req, res, SOAP1) })
