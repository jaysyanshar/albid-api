...
router.post('/', (req, res) => {
    ...
    .then(doc => {
        ...
        if (doc.jenisPemeriksaan != rekamMedisEnums.pemeriksaan.partograf) {
            throw Error('Invalid reference ID.')
        }
        else {
            Partograf.findOne({ idRekamMedis: doc._id }, 'idRekamMedis')
            .then(doc => {
                if (doc == undefined) {
                    crud.createOne(req, res, Partograf)
                }
...
router.put('/', (req, res) => { crud.updateOne(req, res, Partograf) })
router.get('/', (req, res) => { crud.readOne(req, res, Partograf) })
