...
router.post('/', (req, res) => {
    ...
    .then(doc => {
        ...
        if (doc.jenisPemeriksaan != rekamMedisEnums.pemeriksaan.neonatus) {
            throw Error('Invalid reference ID.')
        }
        else {
            Neonatus.findOne({ idRekamMedis: doc._id }, 'idRekamMedis')
            .then(doc => {
                if (doc == undefined) {
                    crud.createOne(req, res, Neonatus)
                }
...
router.put('/', (req, res) => { crud.updateOne(req, res, Neonatus) })
router.get('/', (req, res) => { crud.readOne(req, res, Neonatus) })
