...
router.post('/', (req, res) => {
    ...
    .then(doc => {
        ...
        if (doc.jenisPemeriksaan != rekamMedisEnums.pemeriksaan.pnc) {
            throw Error('Invalid reference ID.')
        }
        else {
            PostnatalCare.findOne({ idRekamMedis: doc._id }, 'idRekamMedis')
            .then(doc => {
                if (doc == undefined) {
                    crud.createOne(req, res, PostnatalCare)
                }
...
router.put('/', (req, res) => { crud.updateOne(req, res, PostnatalCare) })
router.get('/', (req, res) => { crud.readOne(req, res, PostnatalCare) })
