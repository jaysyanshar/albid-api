...
router.post('/', (req, res) => {
    ...
    .then(doc => {
        ...
        if (doc.jenisPemeriksaan != rekamMedisEnums.pemeriksaan.pelayananKesehatan) {
            throw Error('Invalid reference ID.')
        }
        else {
            PelayananKesehatan.findOne({ idRekamMedis: doc._id }, 'idRekamMedis')
            .then(doc => {
                if (doc == undefined) {
                    crud.createOne(req, res, PelayananKesehatan)
                }
...
router.put('/', (req, res) => { crud.updateOne(req, res, PelayananKesehatan) })
router.get('/', (req, res) => { crud.readOne(req, res, PelayananKesehatan) })
