...
router.post('/', (req, res) => {
    ...
    .then(doc => {
        ...
        if (doc.jenisPemeriksaan != rekamMedisEnums.pemeriksaan.soap2) {
            throw Error('Invalid reference ID.')
        }
        else {
            SOAP2.findOne({ idRekamMedis: doc._id }, 'idRekamMedis')
            .then(doc => {
                if (doc == undefined) {
                    crud.createOne(req, res, SOAP2)
                }
...
router.put('/', (req, res) => { crud.updateOne(req, res, SOAP2) })
router.get('/', (req, res) => { crud.readOne(req, res, SOAP2) })
